...
\draw[thick] (0.1, 2.1) coordinate (utc_1) node[above]{UTC} to [out=311,in=169] (3,0.7) coordinate (utc_2);
    ...
    \end{tikzpicture}
    \caption{Individual firm price determination}
\end{figure}


\begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance =-100pt, scale = 4]
        % Draw axes
        \draw [<->, very thick] (0,2) node (yaxis) [above] {Income}
            |- (3,0) node (xaxis) [right] {Labour};
    
        
        \coordinate (lf) at (0.5,0);
            \coordinate (point) at (2.5,1.8);
            \coordinate (40) at (1.25,0);
            
            \draw[dashed]  (40) node[below]{40} -- (1.25, 0.67) --  (0,0.67) node[left]{160\euro};
            \fill[black] (1.25,0.67) circle(0.5pt);
            \node[above left] at (1.25,0.67) {Profit = 0\euro};
    
        
        \node[below] at (2.5,0) (80){80};
        
        
       \node[left] at (0, 1.8) (30){320\euro};
       
       \draw[dashed, thick] (80) -- (point);
       
       
           \draw (lf) -- (point) node [right]{$80 \text{\euro} = P$};
           \draw (lf) -- (2.5, 1) node [right]{$80 \text{\euro} = F$};
                  \draw (lf) -- (2.5, 0.3) node [right]{$80 \text{\euro} = RM + E$};
    
    
           
    \draw[decorate, decoration={brace,mirror,amplitude=10pt}] (0,0) -- (lf) node [black,midway, below, yshift= -1 em] {$L_f$};
    
        
        \end{tikzpicture}
    
    \caption{Income shares in the economy}
\end{figure}
